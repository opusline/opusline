...
This file is the manual for every workspace. The one directory-scoped exception
is `apps/api/CLAUDE.md` (and its byte-identical `AGENTS.md`): below the
`<laravel-boost-guidelines>` tag it is **generated** by `php artisan boost:update`,
...
- **PHP path packages**: a library that needs nothing from the app lives in `apps/api/packages/<name>` (`opusline/<name>`), never in the root `packages/`, which is JS-only — Sail mounts, Turbo hashes, CI filters and the Lefthook PHP hooks all stop at `apps/api`. The API requires it through the `packages/*` path repository; after changing a package's `composer.json`, refresh the lock with `composer update opusline/<name> --no-scripts`. `tests/Unit/ArchitectureTest.php` lists every package with the classes its `require` provides and fails on anything else, the app and undeclared Laravel pieces included. Its tests live in `packages/<name>/tests` and never call `tests/Pest.php` helpers, which would tie it back to the app.
...
- E2E (`apps/e2e`): Playwright over the production compose stack (`apps/e2e/scripts/stack.sh`), in its own CI job and deliberately not a `test` task, so neither `turbo test` nor pre-push boots a stack. One workflow per test, and **every test registers its own account** (the `account` fixture) and provisions through the real API (`support/provision.ts`) — no seeders, no test-only routes, no shared state, so aggregates can be asserted exactly. **Locate by `data-testid` only** — never by role, label, text or CSS, which move with the copy and the markup. An id is a static kebab-case literal (`<feature>-<element>`, greppable from spec to component); whatever varies rides beside it in its own attribute (`data-testid="week-cell" data-mission=… data-date=…`, read with `byTestId` from `support/locators.ts`), and state is exposed as an untranslated token (`data-status="paid"`, `data-minutes`, `data-amount-cents`) rather than read off a badge. Assertions use test-owned data, URLs, input values and those attributes — never UI copy. No `page.clock`: the server cannot follow it — tracking specs navigate to a fixed past week instead.

...
it("files each CRA under what it is waiting on", () => {
  const groups = groupCras("fr-FR", items, "");
...
it("leaves out a group nothing falls into", () => {
  const groups = groupCras("fr-FR", [items[0]], "");
...
it("matches on the mission", () => {
  expect(matchesQuery("fr-FR", items[0], "callisto")).toBe(true);
});

it("matches on the client", () => {
  expect(matchesQuery("fr-FR", items[0], "nordlys")).toBe(true);
});

it("matches on the month as the API writes it", () => {
  expect(matchesQuery("fr-FR", items[0], "2026-07")).toBe(true);
});

it("matches on the month as a human writes it", () => {
  expect(matchesQuery("fr-FR", items[0], "juillet")).toBe(true);
});
...
it("ignores accents in either direction", () => {
  ...
  expect(
    matchesQuery(
      "fr-FR",
      craItem({ missionName: "Refonte éditoriale" }),
      "editoriale",
    ),
  ).toBe(true);
  expect(
    matchesQuery(
      "fr-FR",
      craItem({ missionName: "Refonte editoriale" }),
      "éditoriale",
    ),
  ).toBe(true);
});
...
it("keeps everything when nothing has been typed", () => {
  expect(
    groupCras("fr-FR", items, "   ").flatMap((group) => group.items),
  ).toHaveLength(3);
});

it("narrows to what the search matches", () => {
  const groups = groupCras("fr-FR", items, "lunaprint");

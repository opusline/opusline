...
it("marks the journal busy while another month loads", async () => {
  renderJournal({ isRefreshing: true });
...
it("drops the busy mark once the month is there", async () => {
  renderJournal({ isRefreshing: false });
...
it("names the month when nothing was spent in it", async () => {
  renderJournal({ month: emptyExpensesMonth() });

  expect(await screen.findByText("Aucune dépense en août")).toBeInTheDocument();
  expect(screen.queryByRole("table")).not.toBeInTheDocument();
});

it("lays every expense out both as a table row and as a card", async () => {
  renderJournal();
